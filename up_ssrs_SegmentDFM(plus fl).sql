/*create procedure sb_bio.up_ssrs_SegmentDFM ( 
  ...
  in @Date_fuib_out date,in @Count_month_out smallint default 3, in @Typerep smallint,
    in @Segment varchar(32) ,
   in @RiskFlag integer)*/
...
declare @StateCode_out varchar(32) default '';
declare @Date_fuib_out date;
declare @Count_month_out smallint default 3;
declare @Typerep smallint;
declare @Segment varchar(5) ;
declare @RiskFlag integer;
...
set @RiskFlag = '';
    ...
    select c.Id,
     ...
     case when cv.code = '1001' then 'Микро'
      ...
      where 1 = 1
          ...
          and(@Segment = '' or cv.Code = @Segment)         
  ...
  update ltb_Clients as c
    ...
    c.RiskFlag = cv.Flag,
    c.RiskDesc = case when cv.Flag = 0 then 'Ќизкий уровень риска (' || cr.Value || ')'
    when cv.Flag = 1 then '—редний уровень риска (' || cr.Value || ')'
    when cv.Flag = 2 then '¬ысокий уровень риска (' || cr.Value || ')'
    when cv.Flag = 3 then 'Ќеприемлемо высокий уровень риска (' || cr.Value || ')'
    else null
    end from ltb_Clients as c
    join dfc.tb_sc_vconnotationvalues as cv on c.ClientId = cv.OwnerId and cv.Kind = 243 and (@RiskFlag is null or cv.Flag = @RiskFlag)
  ...
  update ltb_Clients as c
    ...
    ltb_Clients as c
    join ltb_Acc as a on c.ClientID = a.ClientId and a.TopicId in( 2600,2650 ) ;
  commit work;
  ...
  ----------------------------
   select a.Act_lev,
    ...
    a.Segment_DFM
    into #ltb_segment_dfm
    from(                                         --- не зависимо от сегмента???  'Ќеприемлемо высокий уровень риска--нет такого?

          select 1 as Act_lev,'' as Segment /*?*/, 0 as lower_bound,50000 as upper_bound, 0 as RiskFlag,'неактивный низко-рисковый (Ќјнр)' as Segment_DFM   from dbo.iq_dummy union all

      select 1,'',0,50000,1,'неактивный средне-рисковый (Ќјср)' from dbo.iq_dummy union all

      select 1,'',0,50000,2,'неактивный высоко-рисковый (Ќјвр)' from dbo.iq_dummy union all

      select 2,'',50000,400000,0,'малоактивный низко-рисковый (ћјнр)' from dbo.iq_dummy union all

      select 2,'',50000,400000,1,'малоактивный средне-рисковый (ћјср)' from dbo.iq_dummy union all

      select 2,'',50000,400000,2,'малоактивный высоко-рисковый (ћјвр)' from dbo.iq_dummy union all

      select 3,'',400000,1000000,0,'приемлемо-активный низко-рисковый (ѕјнр)' from dbo.iq_dummy union all

      select 3,'',400000,1000000,1,'приемлемо-активный средне-рисковый (ѕјср)' from dbo.iq_dummy union all

      select 3,'',400000,1000000,2,'приемлемо-активный высоко-рисковый (ѕјвр)' from dbo.iq_dummy union all

      select 4,'',1000000,9999999999999,0,'гипер-активный низко-рисковый (√јнр)' from dbo.iq_dummy union all

      select 4,'',1000000,9999999999999,1,'гипер-активный средне-рисковый (√јср)' from dbo.iq_dummy union all

      select 4,'',1000000,9999999999999,2,'гипер-активный высоко-рисковый (√јвр)' from dbo.iq_dummy) as a;
  ...
  /* case when c.kind in (1,4,5,7) then isnull(cv.code,case when DATEDIFF(DD,dateformat(c.Created,'YYYY-MM-DD'),'2018-10-31')<45 then '1002' else '1004' end)
            ...
            when c.kind in (2) then isnull(cv.code,'1')
                ...
                isnull(cv.code,'1')      as Segment,

    case when cv.code='7' then '√руппа'
            when cv.info='”–  ' then '√руппа'
            when cv.code='1004' then 'KKK'
            when cv.code='1001' then 'Mикро'
            when cv.code='1002' then 'MKK'
            when cv.code='1003' then 'CKK'
            when cv.code='1005' then '√ '
            when cv.code in ('2','3','4','5','14','15','16','17','18') then 'VIP'
            when cv.code in ('10','19','20','21','22','23') then 'Affluent'
            when cv.code in ('7','8','9','24','25','26','27','28') then 'Middle'
            when cv.code in ('29','30','31','32','33') then 'LowMid'
            when cv.code in ('1','34','35','36','37','38') then 'Mass'
               else 'N/A' end as SegmentDesc,
      brca.RC
      from DFC.tb_sc_vClients as c
        join DFC.tb_sc_vTreaty as t on c.Id = t.ClientID
        and((t.Closed = 0 and t.ProcessFlag in( 1,3 ) ) or (t.Closed > @Date_int and t.ProcessFlag in( 2 ) ))
...
--        ( 1,2,64,128,1024,2048 )
  ...
  and /*(@Segment = '' or*/ isnull(cv.code,'1')  = @Segment             ----- new param!!
...
select * from ltb_Clients;
  ...
  update ltb_Clients as c
    ...
    join dfc.tb_sc_vconnotationvalues as cv on c.ClientId = cv.OwnerId and cv.Kind = 243  and cv.Flag = @RiskFlag
        join dfc.tb_wb_cfExternalClients as exc on c.ExtId = exc.ExtId and exc.status = 1
    join dfc.tb_wb_cfClientRisk as cr on exc.clientid = cr.clientid
    and cr.status = 1
    and cr.clienttypeid = exc.ExtTypeId
...
---------------------------------
   update ltb_Clients as c
    set isOpenAcc = 1 from
    ltb_Clients as c
    join ltb_Acc as a on c.ClientID = a.ClientId and a.TopicId=2620  ;  ----дл¤ фл!!
  commit work;
  -----------------------
      ---for fiz liz end
  ---------------
...
---------------------------------
  ...
  update ltb_Bills as b
    ...
    ltb_Bills as b
    where 1 = 1
    and b.DebitTopic = 2900               ----дл¤ фл!! ???валютные счета исключать?
    and(right(b.DebitCode,3) <> 'UAH'
    or b.SourceCode = '334851');
  commit work;
    -----------------------
 ...
 select * from #ltb_result;
  ...
  select r.D1,
    ...
    from #ltb_result as r
      left outer join #ltb_segment_dfm as dfm on
          (dfm.segment = r.segment  or dfm.segment= '' )  and
      dfm.RiskFlag = r.riskflag
